...
export default class ManageList extends Component {
		...
		axios
			...
			.then((res) => {
				if (res.data !== "No Login!") {
					...
					alert(
						"Please login first!! Click Back to home button to back"
					);
				}
			})
	...
	setPhoneInfo = async () => {
		...
		axios.post("/user/userPhone", phone).then((res) => {
	...
	getSelect = (val) => {
		...
		axios.post("/user/deletePhone", phone).then((res) => {
	...
	enable = (val) => {
		...
		axios.post("/user/enable", phone).then((res) => {
	...
	disable = (val) => {
		...
		axios.post("/user/disable", phone).then((res) => {
			message("Succes display the item");
		});
	};
    ...
    key: 'seller',
			...
			},
			{
				...
				render: (record) => (
					<>
						<button onClick={() => this.enable(record)}>
							Enable
						</button>
						<button onClick={() => this.disable(record)}>
							Disable
						</button>
						<button onClick={() => this.getSelect(record)}>
							Delete
						</button>
					</>
				),
			...
			<>
				<Table
					...
					expandedRowRender={(record) =>
						expandedRowRender(record.reviews)
					}
				>
